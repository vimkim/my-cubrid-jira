...
**이슈 수행 방안**: 사용자 인용: "current debugging the same (oos.log) for a while" 이므로 `oos.log` 는 보조 debug sink 로 유지한다. 사용자 인용: "for release mode, I want the critical errors and stuff be aligned with CUBRID's own error logging" 이므로 `oos_error` 는 파일 로그 helper 로만 두고, release-critical OOS 오류는 각 실패 지점에서 `er_set` 으로 설정한다. 사용자 인용: "hard fail for now", "CI runs in release mode and I need CI diagnostics for vacuum" 이므로 vacuum 의 HAS_OOS/OOS VFID 불일치는 release 에서 skip 하지 않고 CUBRID 에러 로그를 남긴 뒤 hard fail 한다. error id 는 네 계열로 고정한다: 기존 `ER_HEAP_OOS_OVERPASS_MAXOBJ_SIZE`, 기존 `ER_HEAP_OOS_BAD_INLINE_HEADER`, 신규 `ER_HEAP_OOS_RECORD_CORRUPTED`, 신규 `ER_HEAP_OOS_FILE_MISSING`.
...
| vacuum 진단 hard fail | raw `abort()` 는 release 경로에서 제거한다. 먼저 OOS 전용 error id 와 vacuum context 를 남긴 뒤 CUBRID 오류 경로로 hard fail 한다. | debug 전용 assertion 을 추가로 둘 수 있다. | HAS_OOS record 에서 OOS VFID 가 없는 invariant violation |
...
| vacuum OOS cleanup 에서 OOS VFID 가 없는 경로 | `ER_FATAL_ERROR_SEVERITY` + `vacuum_er_log_error` + 실패 전파 | CI 가 release 빌드에서 vacuum 을 실행하므로 이 invariant violation 은 skip/bounded leak 로 넘기지 않는다. 기존 vacuum loop 가 recoverable page error 처럼 지우고 재시도하는 경로라면, 이 경우만은 진단 로그를 남긴 뒤 hard fail 로 빠지게 한다. |
...
정책은 하나로 고정한다. `OR_MVCC_FLAG_HAS_OOS` 가 설정된 record 에서 OOS VFID 를 찾지 못하면 `ER_HEAP_OOS_FILE_MISSING` 을 `ER_FATAL_ERROR_SEVERITY` 로 설정하고 `vacuum_er_log_error` 로 heap/vacuum context 를 남긴다. 로그에는 최소한 heap file id, slot id, record type, record length, `repid_and_flags`, `mvcc_flags`, `offset_size`, forward-walk 인 경우 undo image 기준 heap VFID 를 포함한다.

그 다음 해당 record 의 OOS cleanup 을 skip 하지 않는다. `vacuum_oos_find_vfid_for_heap_record` 는 error code 를 반환해 현재 page vacuum 을 실패시키고, `vacuum_forward_walk_reclaim_oos` 는 `int` 반환으로 바꾸어 호출자(`vacuum_process_log_block`)가 failure 를 관찰하게 한다. 기존 vacuum loop 가 이 오류를 `er_clear()` 로 지우고 같은 page 를 반복하는 구조라면, 이 invariant violation 은 recoverable cleanup miss 가 아니라 release CI 진단용 hard fail 로 별도 처리한다.
...
- [ ] `vacuum_oos.cpp` 의 develop merge 전 임시 `abort()` 경로가 CUBRID error log + vacuum context + release hard fail 정책으로 대체된다.
...
- vacuum 의 HAS_OOS/OOS VFID 불일치는 임시 진단 목적상 hard fail 한다. transient lookup 실패나 메모리 부족까지 같은 정책으로 묶지는 않는다.
