...
- **영향**: OOS 로 빠지는 값은 정의상 큰 값(`OR_OOS_INLINE_SIZE` = 16바이트 포인터로 바꿀 만큼 큰 값)인데, 그게 압축도 안 된 채 디스크에 쌓인다. 특히 JSON 은 텍스트라서 압축이 잘 먹는데도 통째로 저장돼, OOS 파일 용량과 읽기 I/O 가 불필요하게 커진다.

**이슈 수행 방안**:

- 값을 OOS 파일에 넣기 직전에 압축하고, 읽어올 때 풀어준다. 쓰기는 `heap_attrinfo_insert_to_oos`, 읽기는 `heap_attrvalue_read_oos_inline` 에서 처리한다.
- 타입 자체의 직렬화 코드(`data_writeval`) 안에 압축을 넣는 방식은 쓰지 않는다. 그렇게 하면 OOS 가 아닌 일반 행과 인덱스까지 디스크 포맷이 바뀌어 영향이 너무 커진다 (결정 근거: `docs/adr/0001-oos-payload-compression-at-boundary.md`).
...
- 복제(standby 서버로 복사) 는 압축된 상태 그대로 전송하고, standby 가 같은 코드로 풀어 쓴다 (사용자 결정: "let's go Option 1").
- LZ4 압축은 기존 `cubcompress::compress` / `decompress` (`src/base/compressor.hpp`) 를 재사용한다.
- 압축 on/off 스위치를 정식 시스템 파라미터(`oos_enable_compression`) 로 뺄지 여부: `TBD - 합의 미확인` (현재는 코드 내부 상수, sysprm 화 선호).
...
압축된 값을 "내용물 앞에 쪽지를 붙인 봉투" 라고 보면 된다. 쪽지에는 "압축했음 / 원래 크기는 N" 이 적혀 있다. 값을 저장할 때 봉투에 넣고, 꺼낼 때는 반드시 쪽지를 먼저 읽고 압축을 풀어야 한다. 이 이슈의 핵심 위험은 "값을 꺼내 읽는 곳이 여러 군데인데, 그중 한 곳이라도 쪽지를 안 읽으면 깨진 값을 쓰게 된다" 는 점이다. 그래서 쪽지를 읽는 코드를 한 곳에 모아 모두가 그걸 쓰게 만든다.
...
왜 길목(OOS 경계)에서 하고, 복제는 왜 압축된 채로 보내는지에 대한 선택 근거는 `docs/adr/0001-oos-payload-compression-at-boundary.md` 에 적어 두었다.

## Specification Changes

- OOS 파일에 저장되는 값 앞에 8바이트 헤더(앞서 말한 "쪽지") 가 붙는다. 구성은 압축 방식 1바이트(없음 / LZ4) + 예약 3바이트 + 원래 크기 4바이트다. 예약 3바이트는 나중에 다른 압축 방식을 넣을 자리다.
...
값을 직렬화 -> image (크기 L_u)
  |
  +-> 압축 대상 타입이고, 스위치 ON 이고, L_u >= OOS_MIN_COMPRESS_LEN(255) 이면:
  |     LZ4 로 압축 -> 결과 크기 L_c
  |     이득 있을 때만 채택: (헤더 8B + L_c) < L_u
  |
...
- 설계 결정 기록: `docs/adr/0001-oos-payload-compression-at-boundary.md`.
- 구현 WIP 가 브랜치 `cbrd-26756-oos-compression` 에 있다 (`heap_file.c` 쓰기/읽기, `oos_file.hpp` 공유 코덱, `log_applier.c` 적용기 연동).
- 미결정: 압축 스위치의 sysprm 화 여부 (`oos_enable_compression`).
