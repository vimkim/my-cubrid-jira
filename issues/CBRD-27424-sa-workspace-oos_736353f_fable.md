...
**이슈 수행 방안**: loader와 CSQL이 공유하는 standalone 저장 경로를 수정한다. 현재 `feat/oos` 기준에서 독립적으로 구현하고 PR #7695와의 호환성은 별도로 검증한다. 기존 저장 정책과 no-logging 지원은 유지한다. 기존 정책 적용에 따라 CBRD-26937의 OOS+bigone 거부가 standalone 적재에도 적용된다 (아래 Specification Changes의 동작 변화 고지 참고).
...
**동작 변화 고지**: 기존 OOS 배치 규칙에는 CBRD-26937의 OOS+bigone 거부가 포함된다. 따라서 기존에 `REC_BIGONE`으로 적재되던 레코드(예: 거대 고정 길이 컬럼과 OOS 대상 가변 컬럼의 조합처럼, OOS 분리 후에도 bigone 임계값을 넘는 레코드)는 standalone 적재(`loaddb -S`, workspace 쓰기)에서도 `ER_HEAP_OOS_OVERPASS_MAXOBJ_SIZE` (-1375)로 거부될 수 있다. 이는 일반 SQL INSERT와 동일한 정책을 적용하는 TO-BE의 의도된 결과이며, 거부 시 OOS 조각을 남기지 않는다.
...
`locator_oos_demote_workspace_record` 가 SA 모드의 workspace 레코드를 읽어 기존 heap attribute 변환기에 전달한다.

| 처리 지점 | 구현 |
|---|---|
| 호출 경로 구분 | 공용 API `locator_insert_force`는 `LOCATOR_FORCE_FLAG` 비트마스크의 `LC_FORCE_FLAG_FROM_WORKSPACE`로, 내부 정적 함수는 `from_workspace` 인자로 INSERT, 예약된 OID (객체 식별자)를 채우는 UPDATE, 여러 행 UPDATE를 구분한다. 클래스 정의와 이미 OOS 변환한 레코드는 제외한다. |
...
xlocator_force / locator_force_for_multi_update
  ...
  -> locator_insert_force / locator_update_force
     -> 실제 저장 파티션 선택
        (이동 시 대상 locator_insert_force로 workspace 플래그 전달)
     -> locator_oos_demote_workspace_record
        -> 레코드를 속성 값으로 읽기
...
| 리뷰 반영 커밋 `736353fff` (force 플래그화·헬퍼 개명·헤더 주석 보완) | GCC debug 빌드, OOS CTest 28/28 통과; 2축 재리뷰로 호출부 10곳의 의미 보존 확인 |
| 최신 `feat/oos` (`38093ea85`, PR #7695 병합분 포함) 머지 `39d5e5e5e` | GCC debug 빌드, OOS CTest 36/36 통과 (CLI 회귀 12개 시나리오 포함) |

테스트한 소스 내용은 분석한 PR 커밋과 동일하다. PR #7695 호환성 결과는 빌드와 집중 회귀 검증이며, 해당 PR의 전체 테스트나 CI 통과를 뜻하지 않는다. 이후 PR #7695는 `feat/oos`에 병합되었고(`4f43198c2`), 본 브랜치는 `39d5e5e5e` 머지로 이를 포함한다.
...
- [코드 리뷰(2축) 보고서](https://github.com/vimkim/my-cubrid-docs/blob/861ad4fc647391b561ee156f9ae07d1104d5582b/cbrd-27424/code_review_d4a155a_fable.md)
