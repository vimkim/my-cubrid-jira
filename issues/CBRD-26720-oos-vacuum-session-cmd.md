...
| 명령 | 동작 | 권한 |
|---|---|---|
| `;vacuum` | CS mode: `vacuum_Master_daemon->wakeup()` / SA mode: `xvacuum()` 직접 호출 | DBA group |
...
```
csql.c (S_CMD_VACUUM, DBA group only)
...
- [ ] `;vacuum` 이 DBA group 멤버 csql 세션에서 `NO_ERROR` 로 반환한다 (`--sysadm` 불필요)
- [ ] `;vacuum` 이 일반 유저(미-DBA) 세션에서 "Running vacuum is only allowed for users in the DBA group" 메시지를 출력한다
