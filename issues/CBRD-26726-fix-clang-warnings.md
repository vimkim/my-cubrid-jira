...
| 구분 | 내용 |
|------|------|
| **AS-IS (현재 동작 / 배경)** | Clang 19.1.7 clean build 는 경고 메시지 496건을 출력한다. 이 안에는 `delete`/`new[]` 불일치, `va_start` 의 잘못된 마지막 인자, 항상 참인 비트 조건식처럼 실제 오동작 또는 UB(Undefined Behavior - 언어 표준이 결과를 보장하지 않는 동작) 후보와 생성 코드·외부 코드 경고가 함께 섞여 있다. |
| **TO-BE (목표 상태 / 기대 동작)** | 모든 경고를 담당 하위 이슈와 검증 항목에 연결하고, first-party 코드는 원인을 수정한다. 생성 코드와 외부 코드는 원본·생성기·의존성을 우선 수정하며 불가능할 때만 해당 target 경계에서 격리해 전체 빌드 출력도 0건으로 만든다. |
| **영향** | 기술 부채 - 반복 출력되는 저위험 경고 때문에 `-Wvarargs`, `-Wtautological-bitwise-compare` 같은 결함 신호가 묻히며, 새 경고가 추가되어도 build log 차이만으로 발견하기 어렵다. |

**이슈 수행 방안**: 11개 하위 이슈로 나누어 쉬운 변경부터 처리하는 방안을 권장한다. 전역 `-Wno-*` 추가는 사용하지 않고, 외부 코드의 target 단위 격리 허용 여부와 하위 이슈 번호는 `TBD - 합의 미확인` 으로 남긴다.
...
- **변경 범위 / 영향**: first-party 55개 파일이 `base`, `broker`, `CCI`, `object`, `optimizer`, `parser`, `query`, `storage`, `transaction` 등 17개 모듈에 걸쳐 있다. Flex/Bison 생성 경로와 RapidJSON·dlmalloc·`mprec.c` 도 별도 범위로 포함한다. 공개 API와 디스크 포맷 변경은 의도하지 않지만 CCI/CAS 프로토콜 타입, optimizer 가변 인자 함수, 숫자 경계 판정에는 호환성 검증이 필요하다.
...
현재 작업트리에는 `pt_value_set_monetary()` 의 인자를 `DB_CURRENCY` 에서 `PT_CURRENCY` 로 맞추는 변경이 있다. Parser의 화폐 리터럴은 `PT_CURRENCY_*` 를 전달하고 `PT_VALUE` 도 `PT_CURRENCY` 를 저장하므로 이 경계가 맞다. 이 변경으로 `-Wenum-conversion` 은 현재 로그에서 0건이며, 첫 하위 이슈로 따로 마감한다.
...
> **요지**: 496건 가운데 first-party source diagnostic은 311건이다. 나머지를 단순 제외하면 build log는 계속 시끄럽기 때문에 생성기와 외부 코드에도 명시적인 처리 정책이 필요하다.
...
S1-S4는 서로 독립적으로 진행할 수 있다. S5-S10도 코드 의존성은 작지만 동작 회귀 가능성이 높아 각 하위 이슈에서 테스트를 먼저 확정한다. S7과 S11은 first-party 수정과 병렬로 진행하되, parent 완료 조건인 전체 출력 0건의 마지막 gate가 된다.
...
- [ ] 권장 11개 하위 이슈를 생성하고 현재 경고 종류를 하나 이상 담당 이슈에 연결한다.
