...
**이슈 수행 목적**: 컬럼에 `STORAGE FORCE_OUTLINE` 을 지정하면 해당 가변 타입의 값을 레코드 크기에 관계없이 OOS로 저장하도록 한다. 단, OOS stub(16B) 이하의 값은 외부 저장 이득이 없으므로 inline으로 유지한다.
...
| **TO-BE (목표 상태 / 기대 동작)** | `FORCE_OUTLINE` 이 지정된 가변 컬럼의 non-NULL 값은 일반 OOS trigger(record gate)를 건너뛰고 항상 OOS-backed attribute로 기록된다. 단, 직렬화 크기가 OOS stub(16B) 이하인 값은 stub으로 바꿔도 레코드가 줄지 않으므로 inline으로 유지한다. |
...
- ALTER에서 `STORAGE` 절을 생략하면 기존 정책을 보존한다. 단, `FORCE_OUTLINE` 컬럼을 `FORCE_OUTLINE` 이 허용되지 않는 대상(고정 길이 타입 등)으로 변경하는 경우에는 정책을 자동 제거하고 ALTER를 성공시킨다(의도된 동작, PR #7538 리뷰 반영).
...
`FORCE_OUTLINE` 은 우선순위 힌트가 아니라 강제 배치 정책이다. 해당 컬럼의 값은 행 전체가 작아 일반 demotion이 시작되지 않는 경우에도 OOS로 가야 한다. 다만 16바이트 이하 값을 OOS로 보내면 heap 레코드가 줄지 않거나 오히려 커지므로, 16B 수익성 조건(stub 이하 inline 유지)은 `FORCE_OUTLINE` 에서도 그대로 적용한다(코드 분석 결과 반영, 2026-07-28 코멘트).
...
| `FORCE_OUTLINE` | 미적용 | 적용 | stub(16B)보다 큰 non-NULL 가변 값을 record 크기와 무관하게 항상 OOS에 저장 |
...
| non-NULL, 직렬화 크기 ≤ 16B | stub 치환 이득이 없으므로 inline 유지 |
| NULL | OOS chain을 만들지 않고 NULL 유지 |

### ALTER와 스키마 round-trip

`ALTER ... MODIFY/CHANGE` 에서 `STORAGE` 절을 생략하면 기존 정책을 보존한다. 단, `FORCE_OUTLINE` 컬럼을 `FORCE_OUTLINE` 이 허용되지 않는 대상(고정 길이 타입, 일반 instance attribute가 아닌 경우)으로 변경하면 보존할 정책이 새 도메인에서 유효하지 않으므로 `FORCE_OUTLINE` 을 자동 제거하고 ALTER를 성공시킨다. 이는 의도된 동작이다 — 정책을 그대로 보존하면 `SHOW CREATE TABLE`/unloaddb가 `INT STORAGE FORCE_OUTLINE` 같은 재실행 불가능한 DDL을 출력한다(PR #7538 리뷰 지적). 제거된 정책은 이후 다시 가변 타입으로 되돌려도 자동 복원되지 않으며, 필요하면 `STORAGE FORCE_OUTLINE` 을 다시 지정해야 한다. 명시한 옵션은 다음 상태 전이로 처리한다.
          ...
          └ execute_schema.c
              └ SM_ATTFLAG_OOS_FORCE_OUTLINE
                  ...
                  └ object_representation_sr.c
                      └ OR_ATTRIBUTE::oos_storage (2비트 enum: DEFAULT | PREFER_INLINE | FORCE_OUTLINE)
...
영속 상태는 기존 `SM_ATTFLAG_OOS_PREFER_INLINE` 과 별개인 `SM_ATTFLAG_OOS_FORCE_OUTLINE` 비트를 추가한다. 두 비트는 상호 배타적이어야 한다. CREATE와 ALTER change-map은 한 옵션을 설정할 때 반대편 비트를 제거하고, 절 생략 시 두 비트를 모두 보존한다(단, 위의 비호환 타입 변경 예외에서는 `FORCE_OUTLINE` 을 제거한다). 서버 측 `OR_ATTRIBUTE` 표현은 두 비트필드 대신 하나의 2비트 enum 필드 `oos_storage` 로 통합해 둘 다 set 되는 상태를 표현 자체로 배제한다(PR #7538 리뷰 반영).
...
heap_attrinfo_determine_disk_layout()
  ...
  ├ [2] FORCE_OUTLINE 선처리
  │    └ 가변 + non-NULL + stub(16B)보다 큰 값을 oos_columns[i] = true로 표시
...
강제된 컬럼은 normal 후보 목록에서 제외해 중복 처리하지 않는다. stub(16B) 이하 값은 선처리 대상에서 제외되어 inline으로 남는다. FORCE_OUTLINE 선처리 후의 payload를 기준으로 header offset 크기와 normal demotion 필요 여부를 다시 계산해야 한다.
...
- [ ] 직렬화 크기가 `OR_OOS_INLINE_SIZE`(16B) 이하인 FORCE_OUTLINE non-NULL 가변 값은 inline으로 유지되고 원래 값으로 읽힌다.
...
- [ ] STORAGE 절을 생략하고 FORCE_OUTLINE 컬럼을 비호환 타입(고정 길이 등)으로 MODIFY/CHANGE 하면 정책이 제거되고, SHOW CREATE TABLE/unloaddb DDL이 재실행 가능한 상태를 유지한다.
