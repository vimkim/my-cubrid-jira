...
즉시 고객 장애로 확인된 건은 없다. 결과의 심각성과 발화 확률을 분리해 평가하면 다음과 같고, 대부분이 수년~수개월 잠복해 있었다는 사실 자체가 평상시에는 발화하지 않는다는 방증이다.

| ID | 심각도 | 근거 (결과 / 발화 조건) |
|---|---|---|
| A2 | **High** | 유일한 데이터 정합성급 — vacuum 보호 페이지의 조기 회수 가능. 단 절취(-1)는 타 스레드 보호 등록과 heap scan 이 같은 page 에서 겹쳐야 성립. lock-free fast path 가 2026-01 atomic latch 개편(CBRD-26425)에서 들어온 **최근 회귀**이므로, 이 변경이 포함될 릴리스 이전에 수정하면 고객 노출 없이 끝난다 — 릴리스 일정과 교차 확인 필요. |
| A1 | **High (조건부)** | 걸리면 체크포인트 무한 대기(hang)로 결과는 심각하지만, 트리거가 TDE 암호화 실패 또는 DWB 내부 오류라는 희귀 오류 경로다. TDE 미사용 환경이면 절반은 발화 불가. DWB 비활성화 감지 시 non-DWB 재시도 분기(:10816-10822)는 정상 코드이며 이 결함과 무관하다. |
| A3 | Medium | victim 공급 백업 경로 무력화 — 평시에는 flush 데몬 주 경로가 커버하므로 극한 버퍼 포화에서의 지연 문제. 정합성 훼손 없음. |
| B1 | Medium | 운영 가시성 왜곡 — 기본 설정에서 논리 flush 량을 맞게 답하는 지표가 없고, write 지표는 한쪽 누락·한쪽 이중 집계. 장애 분석·용량 산정의 판단 근거가 틀어진다. |
| B2 | Medium | 대량 스캔 세션이 많은 부하에서 big queue 가 계속 비어 victim 탐색 효율 저하. 기능 오류 아님. |
| C4 | 조사 (미정) | TDE 키 선택 축(temp key vs perm key)이 판정에 걸려 있어, 복구 중 temp 접근 존재 여부 확인 전에는 심각도를 정할 수 없다 — 조사 결과에 따라 High 로 승격될 수 있는 유일한 항목. |
| A4 | Low | 잠재 — 현재 코드 경로·초기화 순서에서는 발화하지 않고, 필드 추가나 초기화 실패 경로 변경 시 결함이 된다. |
| A5 | Low | debug 빌드(CUBRID_DEBUG) 한정 — 릴리스 바이너리 무영향. |
| B3 | Low | 사용성 — 잘못된 값이면 기동 실패로 즉시 드러나므로 조용한 오동작이 없다. |
| B4 | Low | 동작 무변경 정리. |
