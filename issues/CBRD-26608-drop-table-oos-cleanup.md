...
기존 `oos_file_destroy` 는 TODO stub 상태이며, `xheap_destroy` 에서 OOS VFID를 확인하거나
삭제하는 코드가 없었다. 이로 인해 `DROP TABLE` 이후에도 OOS 파일이 디스크에 잔존하여
공간이 회수되지 않는 문제가 있었다.
...
### 5. Recovery handler (`recovery.c`, `recovery.h`, `mvcc.h`)
...
### DELETE 경로에서 OOS 즉시 삭제를 제거한 이유

초기 구현에서는 `locator_delete_oos_force` 를 통해 DELETE 시점에 OOS 레코드를 즉시 물리적으로 삭제하였으나,
MVCC 정합성 문제로 제거하였다. 다른 트랜잭션이 undo log (log LSA recdes)를 통해 이전 버전의 OOS 데이터를
읽을 수 있기 때문에, OOS 레코드는 vacuum이 해당 log LSA를 정리할 때 함께 삭제해야 한다.
...
- DELETE 경로 OOS 정리(`locator_delete_oos_force`)는 MVCC 정합성 문제로 제거됨 — vacuum에서 처리 예정
