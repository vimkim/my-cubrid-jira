# 코멘트 #1 — Analysis · Findings · 참고 코드 · Remarks (보조 자료)

> 본 코멘트는 description 의 "조사 결론" 을 뒷받침하는 코드 인용 / `data_writeval` 콜백별 압축 호출 유무 / OOS 진입 조건 / 타입 매핑 등의 상세 자료다. Description 본문 길이 제약 (≈ 32 KB) 을 피하기 위해 분리.
...
- 위 현황은 description "조사 결론" 의 EXTENDED-at-OOS 구현 시 기준점 자료로 활용한다.
...
- VARCHAR 가 `tp_String` 경로에서 이미 LZ4 압축되는 사실 자체는 EXTENDED-at-OOS
  구현 시 **이중 압축 회피** 분기 설계의 입력이 된다 (description 의 "고려사항"
  참조).
